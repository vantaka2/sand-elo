-- Implement proper Glicko-based team rating system
-- Teams will have their own ratings that evolve based on team performance

-- Drop the old simple averaging function
...
BEGIN
    ...
    -- Store rating changes in team rating history (Type 1 SCD)
...
BEGIN
    ...
    -- Store individual rating changes in rating history
    INSERT INTO rating_history (player_id, match_id, match_type, rating_before, rating_after, rating_change)
    VALUES 
        (t1p1_id, match_id, match_type, t1p1_rating::INT, t1p1_result.new_rating::INT, (t1p1_result.new_rating - t1p1_rating)::INT),
        (t1p2_id, match_id, match_type, t1p2_rating::INT, t1p2_result.new_rating::INT, (t1p2_result.new_rating - t1p2_rating)::INT),
        (t2p1_id, match_id, match_type, t2p1_rating::INT, t2p1_result.new_rating::INT, (t2p1_result.new_rating - t2p1_rating)::INT),
        (t2p2_id, match_id, match_type, t2p2_rating::INT, t2p2_result.new_rating::INT, (t2p2_result.new_rating - t2p2_rating)::INT);
END;
$$ LANGUAGE plpgsql;

-- Update the trigger to properly handle team creation and ratings
...
-- Create function to recalculate all ratings including teams
...
BEGIN
    ...
    FOR iteration IN 1..iterations LOOP
        ...
        -- Clear rating history for clean recalculation
