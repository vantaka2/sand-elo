-- Fix match counts after account linking
-- Update mens_matches_played and womens_matches_played for real users after linking

-- Add match count update to link_cbva_account function
...
-- Add match count update to link_temp_account function
...
-- One-time migration to fix match counts for already linked accounts
DO $$
DECLARE
    user_record RECORD;
    mens_count INT;
    womens_count INT;
BEGIN
    -- Find all real users who might have had accounts linked to them
    FOR user_record IN 
        SELECT DISTINCT p.id 
        FROM profiles p
        WHERE p.account_type = 'real_user'
        AND p.is_active = TRUE
        AND EXISTS (
            SELECT 1 FROM profiles linked 
            WHERE linked.linked_to_profile_id = p.id
        )
    LOOP
        -- Update match counts
        SELECT 
        ...
        INTO mens_count, womens_count
        FROM matches
        WHERE user_record.id IN (team1_player1_id, team1_player2_id, team2_player1_id, team2_player2_id);
        ...
        RAISE NOTICE 'Updated match counts for user %: mens=%, womens=%', 
            user_record.id, mens_count, womens_count;
    END LOOP;
END $$;